...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Train a WaveGAN on a given set of audio')
    ...
    parser.add_argument('--phase-shuffle-shift-factor',
                        dest='shift_factor',
                        type=int,
                        default=2,
    ...
    parser.add_argument(
        '-psb',
        '--phase-shuffle-batchwise',
        dest='batch_shuffle',
        action='store_true',
        help=
        'If true, apply phase shuffle to entire batches rather than individual samples'
    )
    parser.add_argument(
        '-ppfl',
        '--post-proc-filt-len',
    ...
    parser.add_argument('--batches-per-epoch',
                        ...
                        type=int,
                        default=10,
                        help='Batches per training epoch')
    ...
    parser.add_argument(
        ...
        dest='discriminator_updates',
        type=int,
        default=5,
        help='Number of discriminator updates per training iteration')
    ...
    parser.add_argument(
        '-eps',
        '--epochs-per-sample',
        dest='epochs_per_sample',
        type=int,
        default=1,
        help=
        'How many epochs between every set of samples generated for inspection'
    )
    ...
    parser.add_argument('--regularization-factor',
                        dest='lmbda',
                        type=float,
                        default=10.0,
...
if __name__ == '__main__':
    ...
    train_gen, valid_data, test_data = create_dataset(audio_filepaths,
                                                      args['valid_ratio'],
                                                      args['test_ratio'],
    ...
    history_output_path = model_dir / "history.pkl"
    final_discr_metrics_output_path = model_dir / "final_discr_metrics.pkl"
